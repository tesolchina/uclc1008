...
import { SlideContent, UnitTask } from "@/data/types";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Lightbulb, CheckCircle2, XCircle, HelpCircle, BookOpen, ChevronDown, ChevronUp } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-center space-y-1">
        {slide.emoji && (
          <span className="text-3xl block">{slide.emoji}</span>
        )}
      ...
      {/* Main Content Grid: Passage + Task side by side on larger screens */}
      {(hasPassage || hasTask) && (
        <div className={cn(
          "grid gap-4",
          hasPassage && hasTask ? "lg:grid-cols-2" : "grid-cols-1"
        )}>
          {/* Reading Passage Panel */}
          {hasPassage && (
            <Collapsible open={showPassage} onOpenChange={setShowPassage}>
                ...
                <CollapsibleTrigger asChild>
                  <button className="w-full flex items-center justify-between p-3 text-left hover:bg-muted/50 transition-colors rounded-t-lg">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-sm">Reading Passage</span>
                    </div>
                    {showPassage ? (
      ...
      {/* Points (if any, shown when no task) */}
      {slide.points && slide.points.length > 0 && !hasTask && (
      ...
      {/* Points shown alongside task */}
      {slide.points && slide.points.length > 0 && hasTask && !hasPassage && (
        <Card className="bg-card/50">
          <CardContent className="pt-4">
            <ul className="space-y-2">
              {slide.points.map((point, idx) => (
