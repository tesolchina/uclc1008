...
export const precourseAssistantRouter = Router();
...
precourseAssistantRouter.post("/", async (req: Request, res: Response) => {
  try {
    ...
    if (!response.ok) {
      if (response.status === 429) { res.status(429).json({ error: "Rate limit exceeded. Please try again in a moment." }); return; }
      if (response.status === 402) { res.status(402).json({ error: "AI service temporarily unavailable. Please try again later." }); return; }
