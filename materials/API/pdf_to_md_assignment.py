...
def find_pdfs(directory):
    """Find all PDF files in Assignment Materials directory only"""
    assignment_dir = directory / "UE1 (2526B) Teacher Folder" / "03 Assignment Materials"
    pdfs = []
    if assignment_dir.exists():
        for file in assignment_dir.glob("*.pdf"):
            pdfs.append(file)
    return pdfs
...
def main():
    ...
    log("\n" + "="*70)
    log("PDF to Markdown Converter - Assignment Materials Only")
    log("="*70)
    ...
    # Find PDFs in Assignment Materials folder only
    assignment_dir = MATERIALS_DIR / "UE1 (2526B) Teacher Folder" / "03 Assignment Materials"
    log(f"\nScanning for PDFs in: {assignment_dir}")
    pdfs = find_pdfs(MATERIALS_DIR)
    log(f"Found {len(pdfs)} PDF file(s) in Assignment Materials")
